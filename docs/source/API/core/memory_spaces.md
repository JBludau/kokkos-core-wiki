...
`Kokkos::SharedHostPinnedSpace` <sup>since 4.0</sup> is a [`MemorySpace` type](MemorySpaceConcept) alias which is accessible by all enabled [`ExecutionSpace` types](ExecutionSpaceConcept).
The memory stays pinned on the host and is availabe on the device via zero copy access in small chunks (cache lines, memory pages, etc. depending on the backend).
Writes to the memory in one `ExecutionSpace` become visible in other `ExecutionSpaces` at synchronization events. Which events trigger a synchronization depend on the backend specifics.
Nevertheless, fences are synchronization events on all backends.
